...
const craftingRecipes = recipes.filter(r => r.category === 'crafting');

export function CraftingPanel() {
  const { gameState, hasEnoughMaterials, removeFromInventory, addToInventory, addExperience, getMaterial, getItem } = useGame();
  const [activeCrafting, setActiveCrafting] = useState<Map<string, number>>(new Map());
  ...
  const startCrafting = (recipeId: string) => {
    ...
    if (activeCrafting.has(recipeId)) {
      alert('Already crafting this item!');
      return;
    }
    ...
    setActiveCrafting(prev => new Map(prev).set(recipeId, recipe.craftingTime));

    const interval = setInterval(() => {
      setActiveCrafting(prev => {
        ...
        if (current && current > 1) {
          ...
          if (recipe.output.itemId) {
            addToInventory(recipe.output.itemId, recipe.output.quantity, 'item');
          } else if (recipe.output.materialId) {
            addToInventory(recipe.output.materialId, recipe.output.quantity, 'material');
          }
          addExperience(recipe.level * 10);
    ...
    <div className="crafting-panel">
      ...
      <div className="recipes-grid">
        {craftingRecipes.map(recipe => {
          const timeRemaining = activeCrafting.get(recipe.id) || 0;
            ...
            <div key={recipe.id} className={`recipe-card ${!canCraft ? 'disabled' : ''}`}>
              ...
              <div className="recipe-output">
                ...
                {recipe.output.itemId && (
                  <div className="recipe-item output-item">
                    {(() => {
